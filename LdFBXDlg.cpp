// LdFBXDlg.cpp : CLdFBXDlg のインプリメンテーション
...
LRESULT CLdFBXDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	SetWnd();
	ParamsToDlg();
...
LRESULT CLdFBXDlg::OnOK(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
///
	int ret = 0;
...
/////
	...
	if( ret || (tempchar[0] == 0) ){
		MessageBox( "FBXのファイル名が不正です。", "入力エラー", MB_OK );
		return 0;		
	}
	ZeroMemory( name, _MAX_PATH );
	if( tempchar[0] ){
...
LRESULT CLdFBXDlg::OnCancel(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	EndDialog(wID);
	return 0;
}

void CLdFBXDlg::SetWnd()
{
	HWND temphwnd;
...
LRESULT CLdFBXDlg::OnRef(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	OPENFILENAME ofn;
	char buf[_MAX_PATH];
	buf[0] = 0;
