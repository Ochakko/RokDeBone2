// DSKeyDlg.h : CDSKeyDlg の宣言

#ifndef __DSKeyDlg_H_
#define __DSKeyDlg_H_
...
// CDSKeyDlg
class CDSKeyDlg : 
	public CAxDialogImpl<CDSKeyDlg>
{
public:
	enum { IDD = IDD_DSKEYDLG };


	CDSKeyDlg( CMotParamDlg* srcmpdlg );
	~CDSKeyDlg();
...
BEGIN_MSG_MAP(CDSKeyDlg)
	...
	COMMAND_ID_HANDLER(IDC_ALLON, OnAllOn)
	COMMAND_ID_HANDLER(IDC_ALLOFF, OnAllOff)
...
//  LRESULT NotifyHandler(int idCtrl, LPNMHDR pnmh, BOOL& bHandled);
	...
	LRESULT OnAllOn(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);
	LRESULT OnAllOff(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);
...
private:
	...
	int InitList();
	int AddItem2List( int itemno, CDispSwitch* srcds, CDSChange* srcdsc );
