...
int MAXSKINMATRIX = 43;//!!!!!!!!!
int MAXBLURMATRIX = 20;
...
/***
	if( g_KeyBuffer['1'] & 0x80 ){
		...
		morph = melem->morph;
		CShdElem* target;
		target = morph->m_ptarget->selem;

		CMMotElem* mme;
		mme = morph->m_boneelem->GetCurMMotElem( morph->m_baseelem );
		float curvalue;
		curvalue = mme->GetValue( target );
		float newvalue;
		newvalue = curvalue + 0.05f;
		newvalue = min( 1.0f, newvalue );

		mme->SetValue( target, newvalue );
	}
	if( g_KeyBuffer['2'] & 0x80 ){
		CShdElem* melem;
		melem = (*m_shandler)(10);
		CMorph* morph;
		morph = melem->morph;
		...
		float newvalue;
		newvalue = curvalue - 0.05f;
		newvalue = max( 0.0f, newvalue );

		mme->SetValue( target, newvalue );
	}
	if( g_KeyBuffer['3'] & 0x80 ){
		CShdElem* melem;
		melem = (*m_shandler)(10);
		CMorph* morph;
		morph = melem->morph;
		CShdElem* target;
		target = (morph->m_ptarget + 1)->selem;
...
***/
	...
	/***
	//toon1作り直しのテスト
	if( (g_KeyBuffer[VK_SHIFT] & 0x80) && (g_KeyBuffer['T'] & 0x80) ){
		if( m_shandler ){
	...
	***/
	...
	/***
	if( (g_KeyBuffer[VK_SHIFT] & 0x80) && (g_KeyBuffer['S'] & 0x80) ){
		int dlgret;
		CLDSigDlg dlg;
	...
	***/
	...
	if( (g_KeyBuffer[VK_SHIFT] & 0x80) && (g_KeyBuffer['R'] & 0x80) && (g_KeyBuffer['A'] & 0x80) ){
		int dlgret;
		CLDSigDlg dlg;
		dlgret = (int)dlg.DoModal();
		if( dlgret == IDOK ){
	...
	***/
	...
	//if( g_KeyBuffer['Q'] & 0x80 ){
	//	ChangeMotionStep();
	//}

	//if( g_KeyBuffer[VK_MENU] & 0x80 ){
	//	;
	//}
	...
	if( (doinverse == 0) && ((dbgcnt % 10) == 0) && (g_KeyBuffer[VK_SHIFT] & 0x80) && (g_KeyBuffer['I'] & 0x80) ){
		doinverse = 1;
...
***/
	...
	/***
	if( m_shandler && (g_KeyBuffer[VK_SHIFT] & 0x80) && (g_KeyBuffer['I'] & 0x80) && (g_KeyBuffer['E'] & 0x80) ){
		int dlgret;
	...
	***/
	...
	if( m_lbutton == 0 ){
		int keyflag;
		/***
		if( g_KeyBuffer[VK_LEFT] & 0x80 ){
			if( g_KeyBuffer[VK_SHIFT] & 0x80 )
				keyflag = ID_KEY_TARGET_LEFT;
			else
				keyflag = ID_KEY_LEFT;
		}else if( g_KeyBuffer[VK_RIGHT] & 0x80 ){
			if( g_KeyBuffer[VK_SHIFT] & 0x80 )
				keyflag = ID_KEY_TARGET_RIGHT;
			else
				keyflag = ID_KEY_RIGHT;
		}else if( g_KeyBuffer[VK_UP] & 0x80 ){
			if( g_KeyBuffer[VK_SHIFT] & 0x80 ){
				if( g_KeyBuffer['Z'] & 0x80 ){
		...
		}else if( g_KeyBuffer[VK_DOWN] & 0x80 ){
			if( g_KeyBuffer[VK_SHIFT] & 0x80 ){
				if( g_KeyBuffer['Z'] & 0x80 ){
		...
		}else if( g_KeyBuffer[VK_F1] & 0x80 ){
		***/
	...
	if( m_tbb_sha == 0 ){
		if( g_inidata->opt_gdisp && (srcseri == 0) ){
			...
			ret = m_ground_sh->TransformDispData( m_vEyePt, m_ground_mh, &m_matWorld, &m_matView, &m_matProjX, 1, 0 );
			if( ret ){
		...
		if( m_g_sh ){
			...
			ret = m_g_sh->TransformDispData( m_vEyePt, m_g_mh, &m_matWorld, &m_matView, &m_matProjX, 1, 0 );
			if( ret ){
...
int CMyD3DApplication::RenderNormal( LPDIRECT3DSURFACE9 psurf, LPDIRECT3DSURFACE9 pzsurf, D3DVIEWPORT9* pviewport, int presentflag )
{
	...
	if( m_parsdlg->parsmode == PARSMODE_PARS ){
		...
		vw = (float)m_parsdlg->orthosize;
		//vh = (float)m_parsdlg->orthosize * ((float)m_d3dpp.BackBufferHeight / (float)m_d3dpp.BackBufferWidth);
		vh = (float)m_parsdlg->orthosize * ((float)pviewport->Height / (float)pviewport->Width);
...
//	clrc.y2 = m_clirect.bottom;
	...
	if( g_inidata->opt_gdisp ){
		...
		ret = m_ground_sh->Render( 0, m_pd3dDevice, 0, POLYDISP_ALL, m_ground_mh, 0, m_matWorld, m_matView, matProjX, m_vEyePt, 0, 0, NOGLOW );
		if( ret ){
	...
	if( m_g_sh && m_g_disp ){
		ret = m_g_sh->Render( 0, m_pd3dDevice, 0, POLYDISP_ALL, m_g_mh, 0, m_matWorld, m_matView, matProjX, m_vEyePt, 0, 0, NOGLOW );
		if( ret ){
	...
	if( m_ExistData ){
		...
		if( m_tbb_face == 1){
			...
			ret = m_shandler->Render( 0, m_pd3dDevice, 0, g_inidata->opt_polydisp, m_mhandler, g_motdlg->GetMotCookie(), m_matWorld, m_matView, matProjX, m_vEyePt, 0, 0, NOGLOW );
			if( ret ){
			...
			//半透明
			m_pd3dDevice->SetRenderState( D3DRS_FILLMODE, D3DFILL_SOLID );
			ret = m_shandler->Render( 0, m_pd3dDevice, 1, g_inidata->opt_polydisp, m_mhandler, g_motdlg->GetMotCookie(), m_matWorld, m_matView, matProjX, m_vEyePt, 0, 0, NOGLOW );
			if( ret ){
...
//			m_matWorld, m_matView, m_matProjX, m_matProjX, m_smallgband );
	...
	if( m_ExistData && presentflag ){
		...
		if( g_inidata->opt_bmdisp ){
			...
			ret = m_bm_sh->RenderBoneMark( m_pd3dDevice, m_shandler, m_mhandler, iniw, m_matView, matProjX, m_bonelinedlg->m_selcol, m_bonelinedlg->m_unselcol, selboneno );
			if( ret ){
		...
		if( (g_inidata->opt_jointdisp) && (m_selectedno > 0) && (m_tbb_bone == 0 || ((m_tbb_bone == 1) && (m_bonedlg->m_ope == BONEOPE_POS))) ){
			...
			ret = m_select_sh->Render( 0, m_pd3dDevice, 0, POLYDISP_ALL, m_select_mh, 0, m_select_mat, m_matView, matProjX, m_vEyePt, 0, 0, NOGLOW );
			if( ret ){
			...
			ret = m_select_sh->Render( 0, m_pd3dDevice, 1, POLYDISP_ALL, m_select_mh, 0, m_select_mat, m_matView, matProjX, m_vEyePt, 0, 0, NOGLOW );
			if( ret ){
...
//			DWORD dwClipHeight = m_clientHeight/2;
			...
			if( m_paintdlg->m_targetdisp > 0 ){
				...
				if( m_paintdlg->m_brushtype == 1 ){
					...
					ret = m_ei3_sh->Render( 0, m_pd3dDevice, 1, POLYDISP_ALL, m_ei3_mh, 0, worldmat, m_matView, matProjX, m_vEyePt, 0, 0, NOGLOW );
					if( ret ){
...
//////// shadowmap chkinview( normal params )
	if( g_inidata->opt_gdisp ){
		ret = m_ground_sh->ChkInView( m_ground_mh, &m_matWorld, &m_matView, &matProjX );
		_ASSERT( !ret );
	}
	if( m_g_sh ){
		ret = m_g_sh->ChkInView( m_g_mh, &m_matWorld, &m_matView, &matProjX );
		_ASSERT( !ret );
	}
...
////////// shadowmap chkinview and Transform (shadow params)	
	if( g_inidata->opt_gdisp ){
		ret = m_ground_sh->ChkInView( m_ground_mh, &m_matWorld, &m_matViewShadow, &m_matProjShadow );
		_ASSERT( !ret );
		ret = m_ground_sh->TransformDispData( g_sStatus.newpos, m_ground_mh, &m_matWorld, &m_matViewShadow, &m_matProjShadow, 1, 0 );
		_ASSERT( !ret );
	}

	if( m_g_sh ){
		ret = m_g_sh->ChkInView( m_g_mh, &m_matWorld, &m_matViewShadow, &m_matProjShadow );
		_ASSERT( !ret );
		ret = m_g_sh->TransformDispData( g_sStatus.newpos, m_g_mh, &m_matWorld, &m_matViewShadow, &m_matProjShadow, 1, 0 );
		_ASSERT( !ret );
	}
...
///////////////////////// screen chkinview transform (normal params)
	if( g_inidata->opt_gdisp ){
		ret = m_ground_sh->ChkInView( m_ground_mh, &m_matWorld, &m_matView, &matProjX );
		_ASSERT( !ret );
		ret = m_ground_sh->TransformDispData( m_vEyePt, m_ground_mh, &m_matWorld, &m_matView, &matProjX, 1, 0 );
		_ASSERT( !ret );
	}
	...
	if( m_g_sh ){
		ret = m_g_sh->ChkInView( m_g_mh, &m_matWorld, &m_matView, &matProjX );
		_ASSERT( !ret );
		ret = m_g_sh->TransformDispData( m_vEyePt, m_g_mh, &m_matWorld, &m_matView, &matProjX, 1, 0 );
		_ASSERT( !ret );
	}
...
//    m_viewport.Width =  m_clientWidth;
	...
	***/
	...
	UV enduv = { 1.0f, 1.0f };
	m_bgdisp = new CBgDisp2( m_pd3dDevice, g_inidata->bg_tex1, 0, g_inidata->bg_tex2, 0, g_inidata->bg_mvu, g_inidata->bg_mvv, 
...
/***
	...
	CShdElem* firstselem;
	firstselem = (*m_shandler)( 1 );
	_ASSERT( firstselem );

	CShdElem* partselem;
	partselem = (*m_shandler)( serino );

	ret = partselem->LeaveFromChain();
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
	ret = partselem->PutToChain( firstselem, 1 );
	if( ret ){
...
***/
	...
	for( addno = 0; addno < addnum; addno++ ){
		...
		CShdElem* parselem = (*m_shandler)( serino );
		_ASSERT( parselem );
		CShdElem* curselem;
		...
		if( destseri < 0 ){
			...
			ret = Add2Tree( newname, serino + 1 + createcnt, SHDBALLJOINT, depth, 1 );
			if( ret ){
				_ASSERT( 0 );
				return 1;
			}
			...
			ret = ((*m_shandler)( serino + 1 + createcnt ))->SetJointLoc( &temploc );
			if( ret ){
				_ASSERT( 0 );
				return 1;
			}
///////////////
			curselem = (*m_shandler)( serino + 1 + createcnt );
			_ASSERT( curselem );
			...
			ret = curselem->PutToChain( parselem, 1 );
			if( ret ){
				DbgOut( "viewer : NewJoint : PutToChain error !!!\n" );
				_ASSERT( 0 );
				return 1;
...
int CMyD3DApplication::LoadSigFile( char* srcfilename )
{
	m_materialdlg->ShowDlg( SW_HIDE );	
...
////////////	
	...
	D3DXVECTOR3 offsetpos( 0.0f, 0.0f, 0.0f );
	D3DXVECTOR3 rot13( 0.0f, 0.0f, 0.0f );
	ret = sigfile->LoadSigFile( dlg.name, m_ext_th, m_ext_sh, m_ext_mh, 0, dlg.mult, 0, offsetpos, rot13, 0 );
	if( ret ){
		if( ret == D3DAPPERR_MEDIANOTFOUND ){
			return 0;
		}else{
...
int CMyD3DApplication::GetFirstParentNo( int* parnoptr )
{
	int firstparentno;
	firstparentno = 1;//!!!!!
	int seri;
...
int CMyD3DApplication::MakeAllJointTreeFlat( CShdElem** ppfirstparent )
{
	...
	int firstparentno = 1;
	ret = GetFirstParentNo( &firstparentno );
	_ASSERT( !ret );
	int seri;
...
int CMyD3DApplication::LoadMAFile( char* srcfilename )
{
	if( !m_thandler || !m_shandler ){
	...
	ret = (int)::MessageBox( m_hWnd, "moaを読み込むと、読み込み済みの全てのモーションが削除されます。\nよろしいですか？", "確認", MB_OKCANCEL );
	if( ret != IDOK ){						
		return 0;//!!!!!!!!!!!
	}else{
		ret = DestroyMotion( 1 );
...
////////////
	...
	if( destseri < 0 ){
		...
		int createflag = 0;
		ret = Add2Tree( newname, serino, SHDBALLJOINT, depth, 1, createflag );
		if( ret ){
			_ASSERT( 0 );
			return 1;
		}
...
/////////////
	CWriteXFileDlg dlg( srcfilename );
	int dlgret;
	dlgret = (int)dlg.DoModal();
	if( dlgret != IDOK ){
		return 0;
...
////////////
	...
	int ret;
	ret = xfile->WriteXFile( m_sigeditwnd, dlg.xfilename, dlg.mult, dlg.convface, m_thandler, m_shandler, m_mhandler );
	if( ret ){
		delete xfile;
		errormes.errorcode = -1;
		errormes.type = MSGERR_APPMUSTEXIT;
...
int CMyD3DApplication::LoadMotionMulti()
{
	...
	if( g_motdlg && m_mhandler ){
		...
		if( *topchar == 0 ){
			//DbgOut( "%d--%s\r\n", namecnt, buf );
			ret = LoadMotion( buf, dlg.mult, 0, 0, 0, 0, 0, 0 );
			if( ret ){
			...
			while( *topchar != 0 ){
				...
				ret = LoadMotion( fname, dlg.mult, 0, 0, 0, 0, 0, 0 );
				if( ret ){
					DbgOut( "viewer : LoadMotionMulti : LoadMotion error !!!\n" );
					_ASSERT( 0 );
					return 1;
...
int CMyD3DApplication::LoadMotion( char* filename, float quamult, int idling, int ev0idle, int commonid, int forbidnum, int* forbidid, int notfu )
{
	int ret;
...
//			!( (m_tbb_bone == 1) && (m_bonedlg->m_ope == BONEOPE_CREATE) ) ){
		...
		if( m_preview_flag == 0 ){
			...
			if( pickno > 0 ){
				...
				if( (m_tbb_r == 1) || ((m_tbb_ei3 == 1) && (m_paintdlg->m_ope == PAINTOPE_BONE)) || (m_tbb_4 == 1) ){
					m_movebone = pickno;
...
//DbgOut( "viewer : timer : movebone : offset %d %d, ptcursor %d %d\r\n", m_clickoffset.x, m_clickoffset.y, ptCursor.x, ptCursor.y );
				...
				if( (abs( m_clickbone.x - (ptCursor.x - m_clickoffset.x) ) >= 2) || (abs( m_clickbone.y - (ptCursor.y - m_clickoffset.y) ) >= 2) ){
					...
					if( m_tbb_za4 ){
						ret = g_motdlg->MoveBoneZa4( m_matWorld, m_movebone, scx, scy, &lastparent );
						if( ret ){
						...
						ret = g_motdlg->MoveBone( m_matWorld, m_movebone, scx, scy, &lastparent );
						if( ret ){
							DbgOut( "viewer : motdlg MoveBone error !!!\n" );
							_ASSERT( 0 );
							return 1;
			...
			case SELECT_X:
				...
				if( m_select_delta != 0.0f ){
					ret = g_motdlg->RotateBone( m_matWorld, m_movebone, 1, m_select_delta, &lastparent, localflag, m_localq, g_inidata->opt_ikfk, m_tbb_za4 );//Yaxis
					if( ret ){
			...
			case SELECT_Y:
				...
				if( m_select_delta != 0.0f ){
					if( m_tbb_za3 == 0 ){
						ret = g_motdlg->RotateBone( m_matWorld, m_movebone, 2, m_select_delta, &lastparent, localflag, m_localq, g_inidata->opt_ikfk, m_tbb_za4 );//Zaxis
						if( ret ){
							DbgOut( "viewer : motdlg RotateBone error !!!\n" );
							_ASSERT( 0 );
							return 1;
			...
			case SELECT_Z:
				...
				if( m_select_delta != 0.0f ){
					ret = g_motdlg->RotateBone( m_matWorld, m_movebone, 0, m_select_delta, &lastparent, localflag, m_localq, g_inidata->opt_ikfk, m_tbb_za4 );//Xaxis
					if( ret ){
						DbgOut( "viewer : motdlg RotateBone error !!!\n" );
						_ASSERT( 0 );
						return 1;
		...
		//}else if( (m_rbutton == 1) && (m_lbutton == 0) && (m_shiftbone > 0) ){
			...
			((m_shiftbone > 0) && (m_tbb_iti == 0) && (m_tbb_ikt == 0) && (m_tbb_col == 0) && (m_tbb_bone == 0) && 
			...
			case SELECT_CENTER:
				...
				if( (abs( m_clickbone.x - (ptCursor.x - m_clickoffset.x) ) >= 2) || (abs( m_clickbone.y - (ptCursor.y - m_clickoffset.y) ) >= 2) ){
					...
					ret = g_motdlg->ShiftBone( m_shiftbone, (float)( ptCursor.x - m_clickoffset.x ), (float)( ptCursor.y - m_clickoffset.y ), m_tbb_t2, m_tbb_za4 );
					_ASSERT( !ret );
			...
			case SELECT_X:
				...
				ret = g_motdlg->ShiftBone( m_shiftbone, 0, m_select_delta, m_tbb_t2, localflag, m_localq, m_tbb_za4 );
				if( ret ){
			...
			case SELECT_Y:
				...
				ret = g_motdlg->ShiftBone( m_shiftbone, 1, m_select_delta, m_tbb_t2, localflag, m_localq, m_tbb_za4 );
				if( ret ){
					DbgOut( "viewer : motdlg ShiftBone error !!!\n" );
					_ASSERT( 0 );
					return 1;
			...
			case SELECT_Z:
				...
				ret = g_motdlg->ShiftBone( m_shiftbone, 2, m_select_delta, m_tbb_t2, localflag, m_localq, m_tbb_za4 );
				if( ret ){
					DbgOut( "viewer : motdlg ShiftBone error !!!\n" );
					_ASSERT( 0 );
					return 1;
			...
			***/
			...
			((m_shiftbone > 0) && (m_tbb_ikt == 0) && (m_tbb_ei2 == 0) && 
			...
			((m_shiftbone > 0) && (m_tbb_iti == 0) && (m_tbb_ikt == 1) && (m_tbb_col == 0)) ){
...
int CMyD3DApplication::TogglePars()
{
	...
	if( m_parsdlg->parsmode == PARSMODE_PARS ){
		m_parsdlg->parsmode = PARSMODE_ORTHO;
	}else{
		m_parsdlg->parsmode = PARSMODE_PARS;
	}
...
int CMyD3DApplication::SavePoseFile( char* srcfilename )
{
	...
	CFrameCopyDlg fcdlg( m_thandler, m_shandler, m_hWnd );
...
//		DbgOut( "viewer : OnDropFiles : %s\r\n", szFileName );
	...
	if( (moaname[0] == 0) && (quanum > 0) ){
		...
		for( quano = 0; quano < quanum; quano++ ){
			...
			if( *curname ){
				ret = LoadMotion( curname, dlg.mult, 0, 0, 0, 0, 0, 0 );
				if( ret ){
					DbgOut( "viewer : LoadMotionMulti : LoadMotion error !!!\n" );
					_ASSERT( 0 );
					return 1;
...
int CMyD3DApplication::LoadFile()
{
	...
	ofn.hInstance = 0;
	ofn.lpstrFilter = 
(LPCTSTR)"形状(sig(標準),mqo,rok,x)\0*.sig;*.mqo;*.rok;*.x\0モーション(biovision社)bvh\0*.bvh\0ボーンテキスト情報(bon)\0*.bon\0ボーン影響度(im2)\0*.im2\0モーションアクセラレータ(moa)\0*.moa\0制限角度(lim)\0*.lim\0トゥーン１(to1)\0*.to1\0ポーズ(pos)\0*.pos\0";
	ofn.lpstrCustomFilter = NULL;
	ofn.nMaxCustFilter = 0;
	ofn.nFilterIndex = 1;
	ofn.lpstrFile = (LPTSTR)buf;
	...
	if( extptr ){
		int cmpsig, cmprok, cmpmqo, cmpx, cmpbvh, cmpbon, cmpim2, cmpmoa, cmplim, cmpto1, cmppos;
		cmpsig = strcmp( extptr, ".sig" );
		cmprok = strcmp( extptr, ".rok" );
		cmpmqo = strcmp( extptr, ".mqo" );
		cmpx = strcmp( extptr, ".x" );
...
int CMyD3DApplication::SaveFile()
{
	...
	ofn.hInstance = 0;
	ofn.lpstrFilter = 
(LPCTSTR)"形状(標準)sig\0*.sig\0連番mqo\0*.mqo\0マルチトラックXfile\0*.x\0モーション(biovision社)bvh\0*.bvh\0ボーンテキスト情報(bon)\0*.bon\0ボーン影響度(im2)\0*.im2\0制限角度(lim)\0*.lim\0トゥーン１(to1)\0*.to1\0ポーズ(pos)\0*.pos\0";
	ofn.lpstrCustomFilter = NULL;
	ofn.nMaxCustFilter = 0;
	ofn.nFilterIndex = 1;
	ofn.lpstrFile = (LPTSTR)buf;
	...
	ofn.nFileOffset = 0;
	ofn.nFileExtension = 0;
	ofn.lpstrDefExt =(LPCTSTR)"sig\0mqo\0x\0bvh\0bon\0im2\0lim\0to1\0pos\0";
	ofn.lCustData = NULL;
	ofn.lpfnHook = NULL;
	ofn.lpTemplateName = NULL;
	if( GetSaveFileName(&ofn) == 0 ){
	...
	extptr = strrchr( buf, ch );
	if( extptr ){
		int cmpsig, cmpmqo, cmpx, cmpbvh, cmpbon, cmpim2, cmplim, cmpto1, cmppos;
		cmpsig = strcmp( extptr, ".sig" );
		//cmpqua = strcmp( extptr, ".qua" );
		cmpmqo = strcmp( extptr, ".mqo" );
		cmpx = strcmp( extptr, ".x" );
...
int CMyD3DApplication::AddMorphFolder()
{
	int ret;
...
//	g_renderstate[ D3DRS_ALPHAFUNC ] = D3DCMP_GREATEREQUAL;
	...
	if( g_inidata->opt_gdisp ){
		ret = m_ground_sh->Render( 0, m_pd3dDevice, 0, POLYDISP_ALL, m_ground_mh, 0, m_matWorld, m_matView, matProjX, m_vEyePt, 0, 0, WITHGLOW );
		if( ret ){
	...
	if( m_g_sh && m_g_disp ){
		ret = m_g_sh->Render( 0, m_pd3dDevice, 0, POLYDISP_ALL, m_g_mh, 0, m_matWorld, m_matView, matProjX, m_vEyePt, 0, 0, WITHGLOW );
		if( ret ){
	...
	if( m_ExistData ){
		m_pd3dDevice->SetRenderState( D3DRS_FILLMODE, D3DFILL_SOLID );
		ret = m_shandler->Render( 0, m_pd3dDevice, 0, g_inidata->opt_polydisp, m_mhandler, g_motdlg->GetMotCookie(), m_matWorld, m_matView, matProjX, m_vEyePt, 0, 0, WITHGLOW );
		if( ret ){
		...
		//半透明
		ret = m_shandler->Render( 0, m_pd3dDevice, 1, g_inidata->opt_polydisp, m_mhandler, g_motdlg->GetMotCookie(), m_matWorld, m_matView, matProjX, m_vEyePt, 0, 0, WITHGLOW );
		if( ret ){
