// TexKeyDlg.cpp : CTexKeyDlg のインプリメンテーション
#include "stdafx.h"
#include "TexKeyDlg.h"
...
LRESULT CTexKeyDlg::OnAllDelete(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	ret = ::MessageBox( m_hWnd, "全てのテクスチャのキーフレームを削除します。\nよろしいですか？", "確認ダイアログ", MB_OKCANCEL ); 
...
//	}
	...
	LONG lret;
	lret = ::SetWindowLong( m_sl_mp_wnd, GWLP_WNDPROC, (LONG)MPProc2 );
	if( lret == 0 ){
		_ASSERT( 0 );
		return 1;
	}
		...
		if( selectno == 65000 ){
			...
			if( curtk ){
				...
				if( curtc ){
					...
					ListView_SetItemText( m_list_wnd, hititem, TEXSUB_TEXNAME, "設定なし" );
				}
			}
