...
const TCHAR* DXUtil_GetDXSDKMediaPath()
{
    ...
    LONG lResult = RegOpenKeyEx( HKEY_LOCAL_MACHINE, _T("Software\\Microsoft\\DirectX"), 0, KEY_READ, &hKey );
    if( ERROR_SUCCESS != lResult )
        return strNull;

    lResult = RegQueryValueEx( hKey, _T("DX8SDK Samples Path"), NULL, &dwType, (BYTE*)strPath, &dwSize );
    RegCloseKey( hKey );

    if( ERROR_SUCCESS != lResult )
        return strNull;

	_tcscat_s( strPath, MAX_PATH, _T("\\Media\\") );
    return strPath;
}
...
HRESULT DXUtil_FindMediaFile( TCHAR* strPath, TCHAR* strFilename )
{
    ...
    _tcscpy_s( strPath, MAX_PATH, strFilename );
    ...
    // Check if the file exists in the current directory
    _stprintf_s( strPath, MAX_PATH, _T("%s%s"), DXUtil_GetDXSDKMediaPath(), strFilename );
                                 ...
                                 DWORD dwLength, TCHAR* strDefault )
{
    ...
    {
        _tcscpy_s( strValue, MAX_PATH, strDefault );
    }
...
#else
    if( cchDestChar == -1 )
    	strcpy_s( strDestination, MAX_PATH, tstrSource );
    else
    	strncpy_s( strDestination, MAX_PATH, tstrSource, cchDestChar );
#endif
}
...
#else
    if( cchDestChar == -1 )
    	strcpy_s( tstrDestination, MAX_PATH, strSource );
    else
    	strncpy_s( tstrDestination, MAX_PATH, strSource, cchDestChar );
#endif
}
