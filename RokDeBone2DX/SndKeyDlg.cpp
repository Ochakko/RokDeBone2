...
//	CWindow m_elemlist_wnd;
	...
	m_taglist_wnd = GetDlgItem( IDC_TAGLIST );
	m_edittag_wnd = GetDlgItem( IDC_TAGEDIT );
...
LRESULT CSndKeyDlg::OnSelTag(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	if( m_dlg->m_preview_flag == 1 )
		return 0;
...
//bgm
	...
	m_taglist_wnd.SendMessage( LB_RESETCONTENT, 0, 0 );
	GetCurTag( m_animno, m_current, m_curtag );
	if( !m_curtag.empty() ){
		//if( s_dbgcnt < 3 ){
		//	::MessageBox( NULL, "exist tag", "paramstodlg", MB_OK );
		//	s_dbgcnt++;
		//}

		unsigned int tagnum = m_curtag.size();
		int tagno;
		for( tagno = 0; tagno < tagnum; tagno++ ){
			CATag curtag = m_curtag[tagno];
			m_taglist_wnd.SendMessage( LB_ADDSTRING, 0, (LPARAM)(curtag.GetName().c_str()) );
		}
	}
...
int CSndKeyDlg::SetNewWindowProc()
{
	...
	LONG lret;
	lret = ::SetWindowLong( m_sl_mp_wnd, GWLP_WNDPROC, (LONG)MPProcSnd );
	if( lret == 0 ){
		_ASSERT( 0 );
		return 1;
	}
...
LRESULT CSndKeyDlg::OnAddTag(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	//int ret;
	...
	char selname[256] = {0};
	::GetWindowText( m_edittag_wnd, (LPSTR)selname, 256 );

	//::MessageBox( NULL, selname, "addtag", MB_OK );

	unsigned int namelen;
	namelen = strlen( selname );
	if( (namelen > 0) && (namelen < 256) ){
		int ret;
		ret = m_tagh->AddTag( m_animno, m_current, (const double)m_sl_mp_rangemax, selname );
		if( ret ){
			_ASSERT( 0 );
			return 1;
		}
...
LRESULT CSndKeyDlg::OnDelTag(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	int ret;
	...
	int selelem;
	selelem = (int)m_taglist_wnd.SendMessage( LB_GETCURSEL, 0, 0 );
//	if( (selelem < 0) || (selelem == LB_ERR) ){
//		return 0;
//	}
	...
	int lbret;
	unsigned int texlen;
	texlen = (unsigned int)m_taglist_wnd.SendMessage( LB_GETTEXTLEN, selelem, 0 );
	if( texlen >= 256 ){
		_ASSERT(0);
		return 1;
	}
	char selname[256] = {0};
	lbret = (int)m_taglist_wnd.SendMessage( LB_GETTEXT, selelem, (LPARAM)(&selname[0]) );
	if( lbret == LB_ERR ){
		_ASSERT(0);
		return 1;
	}
...
//  lParam = 0;
	...
	ret = m_tagh->DelTag( m_animno, selname );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
...
LRESULT CSndKeyDlg::OnEndTag(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	int ret;
...
//  lParam = 0;
	...
	ret = m_tagh->SetEndTime( m_animno, selname, m_current );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
...
int CSndKeyDlg::WriteSndAnimFile( char* filename )
{
	...
	CSndFile sndfile;
	ret = sndfile.WriteSndFile( filename, anim, curss, m_tagh, m_animno );
	if( ret ){
...
int CSndKeyDlg::LoadSndAnimFile( char* filename, int dlgwarnflag )
{
	...
	CSndFile sndfile;
	CSndAnim* anim = 0;
	ret = sndfile.LoadSndFile( filename, m_sndah, m_app->m_soundbank, &anim, m_tagh );
	if( ret ){
		DbgOut( "SndKeyDlg : LoadSndFile : sndfile LoadSndFile error !!!\n" );
		_ASSERT( 0 );
		return 1;
...
int CSndKeyDlg::GetCurTag( const int animno, const double srctime, std::vector<CATag>& rettag )
{
	rettag.clear();
	...
	int ret;
	ret = m_tagh->GetCurTags( animno, srctime, rettag );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
